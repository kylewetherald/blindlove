...
/**
 * Model representing tokens for email verification and password reset
 */
@Entity
public class TokenAction extends Model {
    ...
    public static final Finder<Long, TokenAction> find = new Finder<>(Long.class, TokenAction.class);

    public static TokenAction findByToken(final String token, final Type type) {
        TokenAction ret = find.where().eq("token", token).eq("type", type).findUnique();
        if (ret != null) {
            if (ret.targetUser != null) {
                ret.targetUser = User.find.byId(ret.targetUser.id);
            }
        }
        return ret;
    }
